...
function convertStep (stepJson, scenarioJson) {
    ...
    switch (stepJson.result.status) {
        case 'passed':
            break;
        case 'failed':
            if (!stepJson.result.error_message) break; // TODO: HACK: For some reason we do not have an error_message sometimes
            stepOutput.push({
                error: [
                    {
                        _attr: {
...
function cucumberJunit (cucumberRaw, options) {
    ...
    return xml({ testsuites: output }, { indent: options.indent });
};
